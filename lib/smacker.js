...
function setupTerminationEvents() {
  ...
  process.on("uncaughtException", (exception) => {
    if (_config.logJson) _log.fatal({ error: "uncaughtException", exception }, "There was an error")
  ...
  process.on("unhandledRejection", (reason) => { // second argument is the promise. for now, it adds no value to the error log
    if (_config.logJson) _log.fatal({ error: "unhandledRejection", reason }, "There was an error")
...
const terminate = (signal) => {
  ...
  if (_config.gracefulShutdownTimeout) {
    timeout = setTimeout(() => {
      if (_config.logJson) _log.info({ signal, gracefulShutdownTimeout: _config.gracefulShutdownTimeout }, "Waited too long for shutdown. Terminating.");
...
module.exports = {
  start: async (service, { config, log } = {}) => {
    ...
    if (_config.gracefulStartupTimeout) {
      timeout = setTimeout(() => {
        if (_config.logJson) _log.info({ gracefulStartupTimeout: _config.gracefulStartupTimeout }, "Waited too long for startup. Terminating.");
